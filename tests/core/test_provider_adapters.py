...
def test_google_parse_candidate_serializes_thought_signature():
    ...
    assert tool_calls[0]["thought_signature"] == f"base64:{base64.b64encode(b'\x01sig').decode('ascii')}"
...
def test_google_convert_messages_maps_tool_result_to_user_function_response():
    ...
    class Part:
        def __init__(self, text=None, function_call=None, thought_signature=None, function_response=None):
            self.text = text
    ...
    types_module = type(
        ...
        (),
        {
            "Part": Part,
            "Content": Content,
            "FunctionCall": lambda name=None, args=None: type("FunctionCall", (), {"name": name, "args": args})(),
